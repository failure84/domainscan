...
<body <?= $this->fetch('tb_body_attrs') ?>>
    <nav class="navbar navbar-dark sticky-top bg-dark flex-md-nowrap p-0">
        ...
        <?= $this->Form->Create('Domains', ['url' =>'domains/index', 'type' => 'get', 'class' => 'w-100']); ?>
        <input class="form-control form-control-dark w-100" type="text" placeholder="Search" name="q" aria-label="Search">
        <?= $this->Form->End(); ?>
        <ul class="navbar-nav px-3">
                <?php if ($this->request->session()->read('Auth.User.username')) { ?>
			        <?php //$this->Html->link(__('Settings'), ['controller' => 'users', 'action' => 'view', $this->request->session()->read('Auth.User.id')], ['class' => 'nav-link']) ?>
